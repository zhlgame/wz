...
<?php } } ?>
        ...
        <div class="container mt-5 fl">
            <div class="sellWindow">
            <?php if($page == 1) { ?><?php echo ad($moduleid,$catid,$kw,6);?><?php } ?>
            <?php if($tags) { ?><?php include template('selllist', 'tag');?><?php } ?>
            </div>
...
<?php echo tag("moduleid=$moduleid&length=20&condition=status=3 and thumb<>''&pagesize=5&order=edittime desc&width=160&height=160&cols=8&template=pro_hot");?> 
            </div>
        </div>
  ...
  <div class="recommend box01 mt-15">
                ...
                <ul class="black">
<?php echo tag("moduleid=$moduleid&table=keyword&condition=moduleid=$moduleid and status=3 and updatetime>$today_endtime-86400*7&pagesize=10&order=week_search desc&key=week_search&template=ssno1");?>
                </ul>       
            </div>
            <div class="clear">
            </div>
