...
<body>
<div><a href="<?php echo $MODULE['1']['linkurl'];?>"><img src="<?php if($DT['logo']) { ?><?php echo $DT['logo'];?><?php } else { ?><?php echo DT_SKIN;?>image/logo.gif<?php } ?>
" alt="<?php echo $DT['sitename'];?>"/></a></div>
<div id="nav">
<?php if(is_array($N)) { foreach($N as $i => $m) { ?>
<a href="<?php echo $EXT['archiver_url'];?><?php echo $m['url'];?>"><?php if($mid==$m['moduleid']) { ?><strong><?php } ?>
<?php echo $m['name'];?><?php if($mid==$m['moduleid']) { ?></strong><?php } ?>
</a>&nbsp;&nbsp;
<?php } } ?>
</div>
