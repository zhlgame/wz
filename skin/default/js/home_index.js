...
(function($) {
	$.fn.jfocus = function(settings) {
		...
		return this.each(function(){
			...
			$this.find("ul").css("width", (sWidth * (len))+2);
			$this.hover(function() {
...
//首页标准模块中间多图广告鼠标触及凸显
	...
	var TT = 0;
	var SPEED = 5000;
	$('#fullScreenSlides li').eq(0).css('z-index', '900');
	$('#fullScreenSlides li').eq(0).css({
		'display': 'block'
	});
	...
	var imgSlides = $('#fullScreenSlides li');
	...
	imgSlides.mouseenter(function() {
		inout = 1;
	})
	imgSlides.mouseleave(function() {
		inout = 0;
	})
	TT = setTimeout(GOGO, SPEED);
$(function(){
